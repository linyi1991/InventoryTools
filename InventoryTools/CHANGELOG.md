...
- 修正繁中遊戲資料中八服皆標記 IsPublic=false，導致「顯示全部繁中伺服器」誤判沒有世界的問題。
- 世界選擇器、物品比價及市場價格相關選單改為保留公開世界或繁中 World ID 4028–4035；不放寬其他隱藏／測試世界，不改已有估價設定。
- 以本機 World 資料核對八服 ID、名稱與旗標，並逐服確認 Universalis HTTP 200；缺少特定物品掛單仍顯示破折號。
- 新增真實資料條件的回歸測試；不修改設定、全域估價或製作流程。

## 13.1.23 - 2026-09-16

- 物品詳情預設並列遊戲 World 資料中可用的繁中伺服器價格（World ID 4028–4035），可取消「顯示全部繁中伺服器」回到原範圍。
- 每服獨立顯示 NQ／HQ 最低價、快取更新時間與在售筆數；沒有資料或沒有該品質售價時顯示破折號，不誤作 0 元。
- 只擴充正在查看物品的查價範圍，不修改全域市場伺服器、背景全庫存預載、Artisan 成本估價或使用者設定。
- 手動更新有 5 秒冷卻；查詢超過 2 分鐘移除等待動畫，保留重新整理入口。資料來自 Universalis 玩家回報，非即時完整市場。

## 13.1.22 - 2026-09-16

- 補齊介面、說明、匯入匯出、角色／雇員、探索視窗、篩選條件、欄位設定、物品來源／用途及動態提示的繁體中文顯示。
- 新增繁中欄位名稱搜尋；保留原始英文搜尋、設定鍵、選項儲存值、命令及 Artisan IPC。
- 移植上游 a3d9479：收藏櫃欄位改用正確的布林篩選控制項。
- 移植上游 4f29500 的獨立修正：以列索引隔離製作項目設定彈窗，避免重複物品互相干擾。
- 修正採集來源附加隱藏／定時／限時標記時未翻譯主名稱的問題；共用靜態後綴表避免重複配置。
- 保留 API13/net9、繁中世界數字 ID 市場查詢、背景手動製作分析及 HQ 分段 Artisan 串接；未直接安裝不相容的 API15 上游套件。
- 本機候選版，建置與靜態檢查不代表遊戲驗收；待使用者測試同意後才發布 Git。

## 13.1.21 - 2026-09-15

- 「模擬製作」把指定製作次數與「計算所有僱員庫存」狀態一併交給 Artisan，不再固定以一件、零起始品質預演。
- 材料依可用 HQ 數量優先分段；HQ 用盡或 HQ/NQ 配比改變時才分析新的起始品質，相同區段只求解一次，999 次製作不會觸發 999 次求解。
- 更改製作數量、按 MAX 或切換僱員庫存後會使舊預演失效，避免以過期配料結果啟動大量製作。
- 製作入口改用成對的 inventory-aware Artisan IPC，讓預演、僱員 HQ 取料、實際配料與自動切職保持一致。
